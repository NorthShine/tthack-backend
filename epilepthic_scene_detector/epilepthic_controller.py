...
def get_corrected_chunk(chunk:list, framerate:int) -> list:
    ...
    Args:
        chunk (list): list with frames
        framerate (int): video framerate

    Returns:
        list: Depending on the received risk value, returns either the processed or the original chunk with frames
    """
    
    risk = get_epilepthic_risk(chunk, framerate)
    ...
    if risk > 0.25:
        ...
        for frame in chunk:
            frame = cv2.normalize(frame, frame, alpha=contrast, beta=brightness, norm_type=cv2.NORM_MINMAX)
